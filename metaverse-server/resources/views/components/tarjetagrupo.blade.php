...
<div class="col-lg-4 col-md-6 col-sm-12 p-1 pb-3 pt-1 m-0 p-3">
    <div class="card text-center">
        <div class="card-header bg-info text-bg-primary">
            Número de participantes: {{ $grupo->participantes->count() }}
        </div>
        <div class="card-body">
            <h5 class="card-title pt-3">{{ $grupo->name }}</h5>
            ...
            <div class="btn-group btn-group-sm pb-4" role="group" aria-label="Basic outlined example">
                @if ($botonver)
                    <a href="{{ route('grupos.show', ["grupo" => $grupo->id]) }}" class="btn btn-outline-secondary">Ver</a>
                @endif


@if (($grupo->propietario_id == auth()->user()->id)  || (auth()->user()->hasRole("admin")))
                        <a href="{{ route("grupos.edit", ["grupo" => $grupo->id]) }}" class="btn btn-outline-success">Editar</a>
                        <a href='{{ route ("grupos.intromasiva",["grupoid" => $grupo->id]) }}' class="btn btn-outline-primary">Intro</a>
                        <button type="submit" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#borrar{{ $grupo->id }}">
                        Borrar
                    </button>
@endif
            </div>
        ...
        <div class="card-footer bg-info text-bg-primary">
